<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Warehouse/Warehouse.Master"
    Inherits="System.Web.Mvc.ViewPage<List<LZL.ForeignTrade.DataEntity.MaterialBuy>>" %>
        ...
        $(document).ready(function() {
            ...
            function loadlistdata(obj, name, value, p) {
                ...
                $.ajax({
                    ...
                    url: '<%=Url.Action("MaterialBuy","Warehouse")%>',
            ...
            $("#Add").bind("click", function() {
                window.location.href = '<%=Url.Content("~/Warehouse/AddMateria/") %>';
            });

            $("#Delete").bind("click", function() {
                var State = confirm('你确认要删除' + $(document).data('checkvalue') + '吗？');
                if (State == true) {
                    window.location.href = '<%=Url.Content("~/Warehouse/DeleteMateria/") %>' + $(document).data('checkvalue');
                }
            });
    ...
    <table width="100%" class="list">
                ...
                <td colspan="2">
                    <%=Html.DropDownList("quyerCondition",
                        new SelectList(new List<SelectListItem>() {
                            new SelectListItem(){ Text="编号", Value ="YclNo"},
        ...
        <tbody listmouse="listmouseovercss">
            ...
            <tr ondblclick="javascript:window.location.href ='<%=Url.Content("~/Warehouse/DetailsMateria/"+Html.Encode(Model[i].ID)) %>'"
                ...
                <td>
                    <%= Html.Encode(Model[i].YclNo)%>
                </td>
                <td>
                    <%= Html.Encode(Model[i].YclType)%>
                </td>
                <td>
                ...
                <td colspan="5" align="right">
                    ...
                    <%
                        ...
                        else
                        {
                            if (p < count)
                            {
                                Response.Write("上一页|" + Html.ActionLink("下一页", "MaterialBuy", "Warehouse", new { page = p + 1 }, null));
                            }
                            else
                            {
                                Response.Write(Html.ActionLink("上一页", "MaterialBuy", "Warehouse", new { page = p - 1 }, null) + "|下一页");
                            }
                        }
