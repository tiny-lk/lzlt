...
<asp:Content ID="MainContent" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <table width="100%"  class="list">
        ...
        <thead align="center">
            ...
            <!-- 报价单信息 -->
            <% 
                Model.InvoiceProduct.Load();
                if (Model.InvoiceProduct.Count > 0)
                {
                    for (int i = 0; i < Model.InvoiceProduct.Count; i++)
                    {
                        Model.InvoiceProduct.ElementAt(i).ProductReference.Load();
                    }
                }
            %>
            <!-- 商品信息 -->
            <tr id="InvoiceProduct♂" style="<%=Model.InvoiceProduct.Count>0?"": "display: none"%>;
                text-align: left;">
                <td colspan="5">
                    <center style="text-align: center; font-size: x-large; font-weight: bolder;">
                        商品信息</center>
                    <!-- 标识子表添加总数 -->
                    <input type="hidden" value='<%= Model.InvoiceProduct.Count %>' name="InvoiceProduct♂regioncount"
                        id="InvoiceProduct♂regioncount" />
                    <!-- 标识子表区域名称(表格名称、实体对象名称) -->
                    ...
                    <fieldset style="<%=Model.InvoiceProduct.Count>0?"": "display: none"%>; width: 95%;">
                        ...
                        <ul id="spxx" style="float: left; width: 100%; clear: both; position: relative;">
                            <%
                                ...
                                {
                                    var htmlstr = "<li style='display: inline;padding:0 2;'>{0}{1}</li>";
                                    var script = @"$(this).parent().remove();display('InvoiceProduct♂','spxx')";
                                    Response.Write(string.Format(htmlstr,
                                        @"<input type='checkbox' name='Product♂ID' checked='true' onclick=" + Html.Encode(script) + " value='" + Model.InvoiceProduct.ElementAt(i).Product.ID.ToString() + "' />",
                                        Html.ActionLink(Model.InvoiceProduct.ElementAt(i).Product.NameCode, "Details", "Product", new { id = Model.InvoiceProduct.ElementAt(i).Product.ID.ToString() }, null)));
                                }
                            %>
                        </ul>
                    </fieldset>
